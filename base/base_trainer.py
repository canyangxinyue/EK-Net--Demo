...
class BaseTrainer:
    def __init__(self, config, model, criterion):
        ...
        if config['distributed'] == True:
            ...
            dtime=torch.tensor(dtime).to(torch.device("cuda",config['local_rank']))
            torch.distributed.broadcast(dtime,src=0)
            dtime=datetime.fromtimestamp(dtime.int())
            torch.cuda.empty_cache()
        curr_time = "{0:%Y%m%d-%H%M%S}".format(dtime)
        dataset_type = config['dataset']['train']['dataset']['type']
        if 'tag' in config.keys():
            tag=os.path.join(config['tag'],dataset_type,curr_time)
        else:
            tag=os.path.join(dataset_type,curr_time)
        self.save_dir = os.path.join(config['trainer']['output_dir'], config['name'],tag)
        ...
        self.optimizer = self._initialize('optimizer', torch.optim, itertools.chain(model.parameters(), criterion.parameters()))
        ...
        if hasattr(self.criterion,"weighting"):
            if hasattr(self.criterion.weighting,"init_param"):
                self.criterion.weighting.device=self.device
                self.criterion.weighting.init_param()
    ...
    def _load_checkpoint(self, checkpoint_path, resume):
        ...
        #     if 'rec_head.rnn' in k  or 'rec_head.embedding' in k :
        #         rm_keys.append(k)
        # for k in rm_keys:
