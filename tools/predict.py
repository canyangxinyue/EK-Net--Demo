...
class Pytorch_model:
    ...
    def predict(self, img_path: str, is_output_polygon=False, short_size: int = 1024):
        ...
        img = self.makeDividable(img,32)
        # img = cv2.resize(img,(1308,736))
        ...
        with torch.no_grad():
            ...
            if len(box_list) > 0:
                ...
                box_list, score_list = [], []
            t = time.time() - start
        return preds[0, :, :, :].detach().cpu().numpy(), box_list, score_list, t, tm, tp
    
    def makeDividable(self,im,divisor=32):
        h, w, _ = im.shape
        dh, dw = divisor-h%divisor , divisor- w%divisor       
...
def save_depoly(model, input, save_path):
    ...
    traced_script_model = optimize_for_mobile(traced_script_model)
    traced_script_model._save_for_lite_interpreter(save_path)
    print(traced_script_model)
...
if __name__ == '__main__':
    ...
    for img_path in tqdm(get_file_list(args.input_folder, p_postfix=['.jpg'])):
        preds, boxes_list, score_list, t ,tm, tp= model.predict(img_path, is_output_polygon=args.polygon)
        # img = draw_bbox(cv2.resize(cv2.imread(img_path)[:, :, ::-1],(1308,736)), boxes_list)
        img = draw_bbox(model.makeDividable(cv2.imread(img_path)[:, :, ::-1]), boxes_list,color=(0, 255, 0))
        totalt,totaltm,totaltp = totalt+t, totaltm+tm, totaltp+tp
        if args.show:
            show_img(preds[0])
        ...
        pred_path = os.path.join(args.output_folder, img_path.stem + '_model_out.jpg')
        cv2.imwrite(output_path, img[:, :, ::-1])

        c,h,w=preds.shape
        figure=np.zeros((c*h,w))
        for i,pred in enumerate(preds):
            figure[i*h: (i+1)*h ,:]=pred
            # pred_path = os.path.join(args.output_folder, img_path.stem + f"[{i}]" + '_pred.jpg')
            # cv2.imwrite(pred_path, pred* 255)
        cv2.imwrite(pred_path,img=figure*255)
        save_result(output_path.replace('_result.jpg', '.txt'), boxes_list, score_list, args.polygon)
        plt.close()
    print(f"total cost:{totalt}, model cost：{totaltm}, post process cost：{totaltp}")
