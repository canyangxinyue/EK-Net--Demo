...
from models import neck
from utils import WarmupPolyLR, runningScore, cal_text_score
from torchvision.transforms import Resize 

#处理为可以被整除的
def resize_dividable(input,divided=32):
    h,w=input.shape[2],input.shape[3]
    hpad = (divided-h%divided)%divided
    wpad = (divided-w%divided)%divided
    img=F.pad(input, pad=(0,wpad,0,hpad), mode="constant",value=0)
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            self.model.save_neck=True
            preds = self.model(batch['img'])
            loss_dict = self.criterion(preds, batch) if not hasattr(self.criterion,"weighting") else self.criterion(preds, batch, self.model, self.optimizer)
            ...
            if self.tensorboard_enable and self.config['local_rank'] == 0:
                ...
                if hasattr(self.criterion,"sigma"): #UW方法专用
                    self.writer.add_scalars("TRAIN/sigma", {f"sigma/_{i}": sigma for i,sigma in enumerate(self.criterion.sigma)}, epoch)
                
                if self.global_step % self.show_images_iter == 0 or self.global_step==1:
                    ...
                    if 'gt_instances' in batch:
                        show_label = torch.cat([show_label,batch['gt_instances'],batch['training_mask_distances'],\
                            batch['gt_distances'][:,0], batch['gt_distances'][:,1], batch['gt_kernel_instances']])
                    show_label = vutils.make_grid(show_label.unsqueeze(1), nrow=cur_batch_size, normalize=False, padding=10, pad_value=1)
                    ...
                    show_pred = vutils.make_grid(show_pred.unsqueeze(1), nrow=cur_batch_size, normalize=False, padding=10, pad_value=1)
                    self.writer.add_image('TRAIN/preds', show_pred, self.global_step)

                    # model output
                    # show_neck_out = []
                    # neck_out=self.model.neck_out
                    # if isinstance(neck_out,list):
                    #     neck_out=neck_out[0]
                    # for kk in range(neck_out.shape[1]//8):
                    #     show_neck_out.append(neck_out[:, kk, :, :])
                    # show_neck_out = torch.cat(show_neck_out)
                    # show_neck_out = vutils.make_grid(show_neck_out.unsqueeze(1), nrow=16, normalize=False, padding=1, pad_value=1)
                    # self.writer.add_image('TRAIN/neck_out', show_neck_out, self.global_step)
                ...
                for key, value in batch.items():
                    if value is not None:
                        if isinstance(value, torch.Tensor):
                            batch[key] = value.to(self.device)
                start = time.time()
                # batch['img']=resize_dividable(batch['img'],divided=128)
