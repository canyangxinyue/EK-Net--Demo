...
class DetectionIoUEvaluator(object):
    ...
    def evaluate_image(self, gt, pred):
        ...
        def get_union(pD, pG):
            # return Polygon(pD).area
            # return Polygon(pD).area + Polygon(pG).area - Polygon(pD).intersection(Polygon(pG)).area
            ...
            if self.is_output_polygon:
                ...
                for gtNum in range(len(gtPols)):
                    for detNum in range(len(detPols)):
                        ...
                        iouMat[gtNum, detNum] = iou_rotate(pD, pG,method='intersection')
            for gtNum in range(len(gtPols)):
                for detNum in range(len(detPols)):
