...
def makeDividable(im,divisor=32):
    h, w, _ = im.shape
...
def draw_bbox_with_text(img_path, result, texts, score_list, color=(0, 0, 255), thickness=2):
    if isinstance(img_path, str):
        img_path = cv2.imread(img_path)
        # img_path = cv2.cvtColor(img_path, cv2.COLOR_BGR2RGB)
    img = img_path.copy()
    for point,text,score in zip(result,texts,score_list):
        point = point.astype(int)
        # cv2.polylines(img, [point], True, color, thickness)
        cv2.fillPoly(img, [point], (0,255,0))
        # img=cv2.putText(img, '{} {:.3f}'.format(text,score), (point[1][0],point[1][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)

        # img=cv2AddChineseText(img, '{} {:.3f}'.format(text,score), (point[0][0],point[0][1]))
        img=cv2AddChineseText(img, '{}'.format(text), (point[0][0],point[0][1]))
    return img
...
# 贪心策略解码函数，解码预测标签
def ctc_greedy_decoder(outputs, ctc_blank=0):
