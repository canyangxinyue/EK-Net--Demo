    ...
    def __call__(self, data: dict) -> dict:
        ...
        for poly, text, tag in zip(text_polys, texts, ignore_tags):
            ...
            if not self.is_poly_outside_rect(poly, 0, 0, w, h):
                ...
                ignore_tags_crop.append(tag if tag else not self.is_poly_in_rect(poly,0,0,w,h))
                texts_crop.append(text)
        data['img'] = img
        data['text_polys'] = np.array([np.float32(text_poly) for text_poly in text_polys_crop])
